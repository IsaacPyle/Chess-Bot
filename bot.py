...
class Bot():
    ...
    def orderMoves(self, moves: List[Move], board: Board) -> List[Move]:
        ...
        for move in moves:
            ...
            moveTuples.append((guess, move))

        rev = sorted(moveTuples, key=lambda x: x[0], reverse=True)
        # if self.extraPrints:
            # print(f"{[guess for guess, _ in moveTuples]} \n{[guess for guess, _ in rev]}\n\n")
        return [move for _, move in rev]
    ...
    def search(self, board: Board, depth: int, alpha: int, beta: int) -> int:
        ...
        moves = self.orderMoves(moves, board)
